...
if initialized == 0
    ...
    updateUIStatus('Connecting aaaato TCP spike server...');
    ...
    for channel = 1:numAmpChannels
        if (channel == 1)
            channelName = ['a-' num2str(chanName(channel) - 1, '%03d')];
        else
            channelName = ['b-' num2str(chanName(channel) - 1, '%03d')];
        end
...
while get(runButtonGroup.Children(2), 'Value')
    ...
    if twaveformdata.BytesAvailable >= waveformBytes10Blocks
        ...
        % Scannel Bale these 10 data blocks
        ...
        filt_data1 = [];
        filt_data2 = [];
        %filt_data_arr = [filt_data1, filt_data2];
        power_bp1 = [];
        power_bp2 = [];
        ...
        for thisPlot = 1:numAmpChannels
            ...
            if thisPlot==1
                ...
                %power_bp1 = [power_bp1 (filt_data1)];
                power_bp1 = abs(hilbert(filt_data1)).^2;
 
                pb1 = mean(power_bp1);
                % when eyes are closed should press space
                fprintf("Channel A " + pb1 + "\n");
                if (pb1 > player1_threshold_up)
                    keys.keyPress(java.awt.event.KeyEvent.VK_A);
                    keys.keyRelease(java.awt.event.KeyEvent.VK_A); 
                end                    
                elseif (pb1 < player1_threshold_down)
                    keys.keyPress(java.awt.event.KeyEvent.VK_S);
                    keys.keyRelease(java.awt.event.KeyEvent.VK_S); 
            elseif thisPlot==2
                filt_data2 = filtfilt(SOS,G,amplifierData(thisPlot,:));
                %power_bp2 = [power_bp2 (filt_data2)];
                power_bp2 = abs(hilbert(filt_data2)).^2;
     
                pb2 = mean(power_bp2);
                fprintf("Channel B " + pb2 + "\n")
                % when eyes are closed should press space
                if (pb2 > player2_threshold_up)
                    keys.keyPress(java.awt.event.KeyEvent.VK_B);
                    keys.keyRelease(java.awt.event.KeyEvent.VK_B); 
                elseif (pb2 < player2_threshold_down)
                    keys.keyPress(java.awt.event.KeyEvent.VK_N);
                    keys.keyRelease(java.awt.event.KeyEvent.VK_N); 
                end
            end 
